...
__cuda_atomic_load(const _Type* __ptr, _Type& __dst, _Order, _Operand, _Sco, __atomic_cuda_mmio_disable)
{
  ...
  uint16_t __value = 0;
  _CUDA_VSTD::__cuda_atomic_load(
    __aligned, __value, _Order{}, __atomic_cuda_operand_b16{}, _Sco{}, __atomic_cuda_mmio_disable{});
...
__cuda_atomic_compare_exchange(_Type* __ptr, _Type& __dst, _Type __cmp, _Type __op, _Order, _Operand, _Sco)
{
  ...
  while (__cmpOffset == (__old & __valueMask))
  {
    ...
    if (_CUDA_VSTD::__cuda_atomic_compare_exchange(
...
_CCCL_DEVICE _Type __cuda_atomic_fetch_update(_Type* __ptr, const _Fn& __op, _Order, _Operand, _Sco)
{
  ...
  while (
    !_CUDA_VSTD::__cuda_atomic_compare_exchange(__ptr, __expected, __expected, __desired, _Order{}, _Operand{}, _Sco{}))
  {
...
__cuda_atomic_store(_Type* __ptr, _Type __val, _Order, _Operand, _Sco, __atomic_cuda_mmio_disable)
{
  ...
  _CUDA_VSTD::__cuda_atomic_fetch_update(
    __ptr,
    [__val](_Type __old) {
      return __val;
    },
...
{
  __dst = _CUDA_VSTD::__cuda_atomic_fetch_update(
    __ptr,
    [__op](_Type __old) {
      return __old + __op;
    },
...
{
  _Tp __expected = _CUDA_VSTD::__atomic_load_n_cuda(__ptr, __ATOMIC_RELAXED, _Sco{});
  _Tp __desired  = __op(__expected);
  while (
    !_CUDA_VSTD::__atomic_compare_exchange_cuda(__ptr, &__expected, __desired, true, __memorder, __memorder, _Sco{}))
  {
    __desired = __op(__expected);
  }
  return __expected;
...
{
  _Tp __ret;
  _CUDA_VSTD::__atomic_load_cuda(__ptr, __ret, __memorder, _Sco{});
  return __ret;
}
template <typename _Tp, typename _Sco>
...
{
  _CUDA_VSTD::__atomic_store_cuda(__ptr, __val, __memorder, _Sco{});
}
template <typename _Tp, typename _Sco>
...
{
  _Tp __ret;
  _CUDA_VSTD::__atomic_exchange_cuda(__ptr, __ret, __val, __memorder, _Sco{});
  return __ret;
}
template <typename _Tp, typename _Sco>
...
{
  return _CUDA_VSTD::__atomic_fetch_update_cuda(
    __ptr,
    [__val](_Tp __old) {
