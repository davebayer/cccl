...
__device__ uint64_t warp_reduce_min(uint64_t __v)
{
  const auto __lo = static_cast<uint32_t>(__v);
  const auto __hi = static_cast<uint32_t>(__v >> 32);

  const auto __ret_hi = __reduce_min_sync(full_warp_mask, __hi);
  const auto __ret_lo = __reduce_min_sync(full_warp_mask, (__ret_hi == __hi) ? __lo : ~0u);

  return (uint64_t{__ret_hi} << 32) | __ret_lo;
}

__global__ void test_warp_reduce_min_kernel(const uint64_t* inputs, uint64_t* outputs)
{
...
bool test_warp_reduce_min(const char* name, const std::array<uint64_t, 32>& inputs)
{
...
int main()
{
  ...
  inputs.fill((uint64_t{2} << 32) | 0u);
  inputs[17] = (uint64_t{1} << 32) | 0xffff'ffffu;
  success &= test_warp_reduce_min("smaller high word wins", inputs);

  inputs[5]  = (uint64_t{1} << 32) | 100u;
  inputs[27] = (uint64_t{1} << 32) | 7u;
  success &= test_warp_reduce_min("smaller low word breaks tie", inputs);
