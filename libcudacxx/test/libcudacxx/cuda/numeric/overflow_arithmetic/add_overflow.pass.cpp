...
__host__ __device__ constexpr void test_add_overflow(Lhs lhs, Rhs rhs, bool overflow)
{
  ...
  {
    const auto result = cuda::add_overflow(lhs, rhs);
  ...
  {
    Result result{};
    const bool has_overflow = cuda::add_overflow(lhs, rhs, result);
...
__host__ __device__ constexpr void test_type()
{
  ...
  ASSERT_SAME_TYPE(decltype(cuda::add_overflow(Lhs{}, Rhs{})), cuda::overflow_result<Result>);
  static_assert(noexcept(cuda::add_overflow(Lhs{}, Rhs{})));

  ASSERT_SAME_TYPE(decltype(cuda::add_overflow(Lhs{}, Rhs{}, cuda::std::declval<Result&>())), bool);
  static_assert(noexcept(cuda::add_overflow(Lhs{}, Rhs{}, cuda::std::declval<Result&>())));
