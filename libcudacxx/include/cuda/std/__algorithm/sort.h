...
_LIBCUDACXX_HIDE_FROM_ABI _CCCL_CONSTEXPR_CXX14 unsigned
  ...
  _RandomAccessIterator& __first, _RandomAccessIterator& __lm1, uint64_t& __left_bitset, uint64_t& __right_bitset)
{
  ...
  if (__left_bitset)
  {
    ...
    {
      difference_type __tz_left = __detail::__block_size - 1 - _CUDA_VSTD::__libcpp_clz(__left_bitset);
  ...
  else if (__right_bitset)
  {
    ...
    {
      difference_type __tz_right = __detail::__block_size - 1 - _CUDA_VSTD::__libcpp_clz(__right_bitset);
...
_LIBCUDACXX_HIDE_FROM_ABI _Number __log2i(_Number __n)
{
  ...
  {
    return sizeof(unsigned) * CHAR_BIT - 1 - _CUDA_VSTD::__libcpp_clz(static_cast<unsigned>(__n));
  }
  if (sizeof(__n) <= sizeof(unsigned long))
  {
    return sizeof(unsigned long) * CHAR_BIT - 1 - _CUDA_VSTD::__libcpp_clz(static_cast<unsigned long>(__n));
  }
...
_LIBCUDACXX_HIDE_FROM_ABI _CCCL_CONSTEXPR_CXX14 void sort(_RandomAccessIterator __first, _RandomAccessIterator __last)
{
