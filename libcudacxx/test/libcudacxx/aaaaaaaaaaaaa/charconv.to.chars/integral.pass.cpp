...
#if _CCCL_HAS_INT128()
__host__ __device__ constexpr __uint128_t make_u128(__uint128_t a, cuda::std::uint64_t b)
{
...
__host__ __device__ constexpr __uint128_t make_u128(__uint128_t a, cuda::std::uint64_t b, cuda::std::uint64_t c)
{
  a *= 10000000000000ULL;
  a += b;
  a *= 10000000000000ULL;
...
__host__ __device__ constexpr __int128_t make_i128(__int128_t a, cuda::std::int64_t b)
{
  if (a < 0)
  {
...
__host__ __device__ constexpr __int128_t make_i128(__int128_t a, __int128_t b, cuda::std::int64_t c)
{
  if (a < 0)
  {
...
struct test_basics : to_chars_test_base<T>
{
  ...
  __host__ __device__ constexpr void operator()()
  {
    ...
    for (int b = 2; b < 37; ++b)
    {
      using xl = cuda::std::numeric_limits<T>;
