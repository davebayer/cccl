...
__device__ __int128_t warp_reduce_max(__int128_t __v)
{
  ...
  const auto __r3 = static_cast<uint32_t>(__reduce_max_sync(full_warp_mask, static_cast<int32_t>(__w3)));
...
__global__ void test_warp_reduce_max_kernel(const __int128_t* inputs, __int128_t* outputs)
{
...
bool test_warp_reduce_max(const char* name, const std::array<__int128_t, 32>& inputs)
{
  ...
  __int128_t expected = static_cast<__int128_t>(__uint128_t{1} << 127);
...
int main()
{
  ...
  for (unsigned sign = 0; sign < 2; ++sign)
  {
    ...
    for (unsigned word = 0; word < 4; ++word)
    {
      ...
      const auto winner_word = word == 3 && sign != 0 ? 0x8000'0003u : 3u;
      ...
      std::snprintf(name, sizeof(name), "%s word %u decides maximum", sign == 0 ? "positive" : "negative", word);
      success &= test_warp_reduce_max(name, inputs);
    }
  }

  inputs.fill(minimum);
  inputs[3]  = maximum - 123;
  inputs[23] = inputs[3];
  success &= test_warp_reduce_max("tied positive maximum", inputs);

  for (unsigned winner = 0; winner < inputs.size(); ++winner)
  {
    inputs.fill(minimum);
    inputs[winner] = maximum - winner;
