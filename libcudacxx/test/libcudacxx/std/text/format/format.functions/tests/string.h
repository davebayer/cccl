...
TEST_FUNC void test_string(const W& world, const U& universe)
{
  ...
  // todo(dabayer): These don't pass.
  // assert(check_exception("Using manual argument numbering in automatic argument numbering mode", SV("hello {:{0}}"),
  // world, 1)); assert(check_exception("Using automatic argument numbering in manual argument numbering mode",
  // SV("hello {0:{}}"), world, 1)); Arg-id may not have leading zeros.
