...
__host__ __device__ constexpr bool constexpr_test()
{
  ...
  static_assert(cuda::countl_one(nl::max()) == nl::digits);
  static_assert(cuda::countl_one(T(nl::max() - 1)) == nl::digits - 1);
  static_assert(cuda::countl_one(T(nl::max() - 2)) == nl::digits - 2);
  static_assert(cuda::countl_one(T(nl::max() - 3)) == nl::digits - 2);
  static_assert(cuda::countl_one(T(nl::max() - 4)) == nl::digits - 3);
  static_assert(cuda::countl_one(T(nl::max() - 5)) == nl::digits - 3);
  static_assert(cuda::countl_one(T(nl::max() - 6)) == nl::digits - 3);
  static_assert(cuda::countl_one(T(nl::max() - 7)) == nl::digits - 3);
  static_assert(cuda::countl_one(T(nl::max() - 8)) == nl::digits - 4);
  static_assert(cuda::countl_one(T(nl::max() - 9)) == nl::digits - 4);
...
{
  volatile auto v = val;
  assert(cuda::countl_one(v) == expected);
}

template <typename T>
__host__ __device__ void runtime_test()
{
  static_assert(cuda::std::is_same_v<int, decltype(cuda::countl_one(T(0)))>);
  static_assert(noexcept(cuda::countl_one(T(0))));
...
int main(int, char**)
{
...

  {
    ...
    val <<= 32;
    assert(cuda::countl_one(~val) == dig - 40);
    val <<= 2;
    assert(cuda::countl_one(~val) == dig - 42);
    val <<= 3;
    assert(cuda::countl_one(~val) == dig - 45);
  }
#endif // _CCCL_HAS_INT128()
