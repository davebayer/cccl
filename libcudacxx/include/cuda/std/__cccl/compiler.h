...
#  define _CCCL_CUDA_ARCH          __CUDA_ARCH__
#elif defined(_NVHPC_CUDA)
#  define _CCCL_CUDA_COMPILER_NVHPC _CCCL_CUDA_COMPILER_MAKE_VERSION(__CUDACC_VER_MAJOR__, __CUDACC_VER_MINOR__)
#  define _CCCL_CUDA_ARCH           __NVCOMPILER_CUDA_ARCH__
#elif defined(__CUDA__) && _CCCL_COMPILER(CLANG)
#  define _CCCL_CUDA_COMPILER_CLANG _CCCL_CUDACC_MAKE_VERSION(CUDA_VERSION / 1000, (CUDA_VERSION % 1000) / 10)
#  define _CCCL_CUDA_ARCH           __CUDA_ARCH__
#elif defined(__CUDACC_RTC__)
...
#  define _CCCL_CUDA_ARCH           __CUDA_ARCH__
  ...
  || defined(_CCCL_CUDA_COMPILER_NVRTC)
// Shorthand to check whether there is a cuda compiler available
#  define _CCCL_HAS_CUDA_COMPILER 1

#  define _CCCL_CUDA_COMPILER_COMPARE_VERSION_1(_COMP) _COMP
#  define _CCCL_CUDA_COMPILER_COMPARE_VERSION_3(_COMP, _OP, _MAJOR) \
    _CCCL_CUDA_COMPILER_COMPARE_VERSION_4(_COMP, _OP, _MAJOR, 0)
#  define _CCCL_CUDA_COMPILER_COMPARE_VERSION_4(_COMP, _OP, _MAJOR, _MINOR) \
    (_COMP && (_COMP _OP _CCCL_CUDA_COMPILER_MAKE_VERSION(_MAJOR, _MINOR)))

#  define _CCCL_CUDA_COMPILER_SELECT_COUNT(_ARG1, _ARG2, _ARG3, _ARG4, _ARG5, ...) _ARG5
#  define _CCCL_CUDA_COMPILER_SELECT2(_ARGS)                                       _CCCL_CUDA_COMPILER_SELECT_COUNT _ARGS
// MSVC traditonal preprocessor requires an extra level of indirection
#  define _CCCL_CUDA_COMPILER_SELECT(...)        \
    _CCCL_CUDA_COMPILER_SELECT_COUNT(            \
      __VA_ARGS__,                               \
      _CCCL_CUDA_COMPILER_COMPARE_VERSION_4,     \
      _CCCL_CUDA_COMPILER_COMPARE_VERSION_3,     \
      _CCCL_CUDA_COMPILER_COMPARE_BAD_ARG_COUNT, \
      _CCCL_CUDA_COMPILER_COMPARE_VERSION_1,     \
      _CCCL_CUDA_COMPILER_COMPARE_BAD_ARG_COUNT)
#  define _CCCL_CUDA_COMPILER(...) \
    _CCCL_CUDA_COMPILER_SELECT(_CCCL_CUDA_COMPILER_##__VA_ARGS__)(_CCCL_CUDA_COMPILER_##__VA_ARGS__)
#else
#  define _CCCL_CUDA_COMPILER(_COMP) 0
#endif // cuda compiler available

#if defined(_CCCL_CUDA_ARCH)
#  define _CCCL_CUDA_COMPILATION 1
#  if defined(__CUDA_ARCH_LIST__)
#    define _CCCL_CUDA_ARCH_LIST __CUDA_ARCH_LIST__
#  endif
#endif
