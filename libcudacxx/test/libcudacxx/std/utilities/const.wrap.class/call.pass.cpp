...
struct OverloadSet
{
  ...
  constexpr int operator()(std::constant_wrapper<42>) const
  {
    return 2;
  }
};
...
{
  constexpr NonStructural operator()(int i) const
  {
...
constexpr int nothrow_call(int) noexcept
{
  return 42;
}

constexpr int throwing_call(int)
{
  return 42;
}
...
struct S
{
  ...
  constexpr int mem_fun(int i) const
  {
    return member + i;
  }
};
