...
public:
  ...
  _CCCL_API void push_back(_CharT __c)
  {
    ...
    _CCCL_ASSERT(__ptr_ && __size_ < __capacity_ && __available() >= 1, "attempted to write outside the buffer");
...
struct __fmt_back_insert_iterator_container<back_insert_iterator<_Container>, enable_if_t<__fmt_insertable<_Container>>>
{
...
public:
  ...
  [[nodiscard]] _CCCL_API auto __out_it() &&
  {
    ...
    return back_insert_iterator{*__container_};
  }
...
public:
  ...
  [[nodiscard]] _CCCL_API auto __out_it() &&
  {
    return ::cuda::std::copy(this->__view().begin(), this->__view().end(), ::cuda::std::move(__out_it_)).out;
  }
...
template <class _OutIt, class _CharT>
class __fmt_buffer_selector
{
  using _Container _CCCL_NODEBUG_ALIAS = typename __fmt_back_insert_iterator_container<_OutIt>::type;
...
template <class _OutIt, class _CharT>
class __fmt_format_to_n_buffer : private __fmt_buffer_selector<_OutIt, _CharT>::type
{
public:
...
// discarded.
template <class _CharT>
class __fmt_formatted_size_buffer : private __fmt_output_buffer<_CharT>
{
public:
  ...
  [[nodiscard]] _CCCL_API size_t __result() &&
  {
...
public:
  ...
  struct __iterator
  {
    ...
    _CCCL_API constexpr explicit __iterator(__fmt_retarget_buffer& __buffer)
        : __buffer_(::cuda::std::addressof(__buffer))
    {}
  ...
  _CCCL_API __iterator __make_output_iterator()
  {
  ...
  _CCCL_API basic_string_view<_CharT> __view()
  {
