...
public:
  ...
  _CCCL_API void push_back(_CharT __c)
  {
    ...
    // printf("size: %lu, capacity %lu, available %lu\n", __size_, __capacity_, __available());
    // _CCCL_ASSERT(__ptr_ != nullptr && __size_ < __capacity_ && __available() >= 1, "attempted to write outside the
    // buffer");
    _CCCL_ASSERT(__ptr_ != nullptr, "");
    _CCCL_ASSERT(__size_ < __capacity_, "");
    _CCCL_ASSERT(__available() >= 1, "attempted to write outside the buffer");
