...
{
  static_assert(!::cuda::std::is_same_v<typename _Config::hierarchy_type, no_init_t>,
                "Can't launch a configuration without hierarchy dimensions");
  ::CUlaunchConfig __config{};
  constexpr bool __has_cluster_level        = has_level_v<cluster_level, typename _Config::hierarchy_type>;
  ...
  __config.blockDimX = block.dims(block, __conf).x;
  __config.blockDimY = block.dims(block, __conf).y;
  __config.blockDimZ = block.dims(block, __conf).z;
