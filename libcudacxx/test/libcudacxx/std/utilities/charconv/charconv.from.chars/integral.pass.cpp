...
__host__ __device__ constexpr void test_from_chars(const TestItem& item)
{
  using U = cuda::std::make_unsigned_t<T>;

  static_assert(cuda::std::is_same_v<cuda::std::from_chars_result,
                                     decltype(cuda::std::from_chars((const char*) {}, (const char*) {}, T&, Base))>);
  static_assert(cuda::std::is_same_v<cuda::std::from_chars_result,
                                     decltype(cuda::std::from_chars((const char*) {}, (const char*) {}, U&, Base))>);

  static_assert(noexcept(cuda::std::from_chars((const char*) {}, (const char*) {}, T&, Base)));
  static_assert(noexcept(cuda::std::from_chars((const char*) {}, (const char*) {}, U&, Base)));

  // Test signed
  if (cuda::std::in_range<T>(item.val))
  {
    char buff[buff_size]{};
    const auto result = cuda::std::from_chars(buff, buff + buff_size, static_cast<T>(item.val), Base);
    assert(result.ec == cuda::std::errc{});
    assert(result.ptr == buff + str_len(item.str_signed));
  ...
  {
    char buff[buff_size]{};
    const auto result = cuda::std::to_chars(buff, buff + buff_size, static_cast<U>(uint64_t(item.val)), Base);
    assert(result.ec == cuda::std::errc{});
...
__host__ __device__ constexpr void test_base()
{
  ...
  {
    test_from_chars<int8_t, Base>(item);
    test_from_chars<int16_t, Base>(item);
    test_from_chars<int32_t, Base>(item);
    test_from_chars<int64_t, Base>(item);
  }
}
