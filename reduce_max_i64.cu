...
__device__ int64_t warp_reduce_max(int64_t __v)
{
  // Flip the sign bit so unsigned max orders the high word as signed.
  const auto __lo = static_cast<uint32_t>(__v);
  const auto __hi = static_cast<uint32_t>(static_cast<uint64_t>(__v) >> 32);

  const auto __max_hi = static_cast<uint32_t>(__reduce_max_sync(full_warp_mask, static_cast<int32_t>(__hi)));
  const auto __max_lo = __reduce_max_sync(full_warp_mask, (__max_hi == __hi) ? __lo : 0u);

  return static_cast<int64_t>((uint64_t{__max_hi} << 32) | __max_lo);
}

__global__ void test_warp_reduce_max_kernel(const int64_t* inputs, int64_t* outputs)
{
...
bool test_warp_reduce_max(const char* name, const std::array<int64_t, 32>& inputs)
{
...
int main()
{
  ...
  success &= test_warp_reduce_max("all minimum", inputs);

  inputs.fill(-1);
  success &= test_warp_reduce_max("all negative ones", inputs);

  inputs.fill(std::numeric_limits<int64_t>::min());
  inputs[17] = std::numeric_limits<int64_t>::max();
  success &= test_warp_reduce_max("positive high word wins", inputs);

  inputs.fill(-1);
  inputs[9] = 0;
  success &= test_warp_reduce_max("zero versus negative", inputs);

  inputs.fill(static_cast<int64_t>(0x8000'0000'ffff'ffffull));
  inputs[17] = static_cast<int64_t>(0x8000'0001'0000'0000ull);
  success &= test_warp_reduce_max("signed high word dominates low word", inputs);
  ...
  success &= test_warp_reduce_max("negative high word tie", inputs);

  inputs[11] = inputs[5];
  success &= test_warp_reduce_max("multiple maximum lanes", inputs);

  inputs.fill(0x0000'0001'0000'0000ll);
  inputs[5]  = 0x0000'0001'0000'0064ll;
  inputs[27] = 0x0000'0001'0000'0007ll;
  success &= test_warp_reduce_max("positive high word tie", inputs);

  char name[64];
  for (unsigned winner = 0; winner < inputs.size(); ++winner)
  {
    inputs.fill(std::numeric_limits<int64_t>::min());
    inputs[winner] = std::numeric_limits<int64_t>::max() - winner;
