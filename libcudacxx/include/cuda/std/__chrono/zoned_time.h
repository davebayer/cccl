...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI explicit zoned_time(_TimeZonePtr __zone)
      : __zone_{std::move(__zone)}
      , __tp_{}
  {}
  ...
  _LIBCPP_HIDE_FROM_ABI explicit zoned_time(string_view __name)
    requires(requires { __traits::locate_zone(string_view{}); }
      ...
      : __zone_{__traits::locate_zone(__name)}
      , __tp_{}
  {}
  ...
  _LIBCPP_HIDE_FROM_ABI zoned_time(const zoned_time<_Duration2, _TimeZonePtr>& __zt)
    requires is_convertible_v<sys_time<_Duration2>, sys_time<_Duration>>
      : __zone_{__zt.get_time_zone()}
      , __tp_{__zt.get_sys_time()}
  {}
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI operator sys_time<duration>() const
  {
    return get_sys_time();
  }
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI explicit operator local_time<duration>() const
  {
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI local_time<duration> get_local_time() const
  {
    return __zone_->to_local(__tp_);
  }
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI sys_time<duration> get_sys_time() const
  {
    return __tp_;
  }
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI sys_info get_info() const
  {
...
using __time_zone_representation _LIBCPP_NODEBUG =
  conditional_t<is_convertible_v<_TimeZonePtrOrName, string_view>, const time_zone*, remove_cvref_t<_TimeZonePtrOrName>>;
