...
template <class _CharT, class _Traits>
_LIBCPP_HIDE_FROM_ABI basic_ostream<_CharT, _Traits>& operator<<(basic_ostream<_CharT, _Traits>& __os, const month& __m)
{
  return __os
      << (__m.ok() ? std::format(__os.getloc(), _LIBCPP_STATICALLY_WIDEN(_CharT, "{:L%b}"), __m)
...
template <class _CharT, class _Traits>
_LIBCPP_HIDE_FROM_ABI basic_ostream<_CharT, _Traits>& operator<<(basic_ostream<_CharT, _Traits>& __os, const year& __y)
{
...
{
  return __os
      << (__wd.ok() ? std::format(__os.getloc(), _LIBCPP_STATICALLY_WIDEN(_CharT, "{:L%a}"), __wd)
...
operator<<(basic_ostream<_CharT, _Traits>& __os, const weekday_indexed& __wdi)
{
  ...
  return __os
      << (__i >= 1 && __i <= 5
            ...
            : std::format(
                __os.getloc(), _LIBCPP_STATICALLY_WIDEN(_CharT, "{:L}[{} is not a valid index]"), __wdi.weekday(), __i));
}
...
operator<<(basic_ostream<_CharT, _Traits>& __os, const local_info& __info)
{
  ...
  return __os
      << std::format(_LIBCPP_STATICALLY_WIDEN(_CharT, "{}: {{{}, {}}}"), __result(), __info.first, __info.second);
}
