...
TEST_FUNC _CCCL_NOINLINE constexpr bool
check(cuda::std::basic_string_view<CharT> expected, cuda::std::basic_string_view<CharT> fmt, Args&&... args)
{
  assert(expected.size() < 4096 && "Update the size of the buffer.");
  {
    ...
    cuda::std::vformat_to(cuda::std::back_inserter(out), fmt, cuda::std::make_format_args<context_t<CharT>>(args...));
...
template <class CharT, class... Args>
TEST_FUNC _CCCL_NOINLINE constexpr bool check_exception(
  ...
  [[maybe_unused]] Args&&... args)
{
  ...
  NV_IF_TARGET(NV_IS_HOST, ({
                 try
                 {
                   ...
                   cuda::std::vformat_to(
                     cuda::std::back_inserter(out), fmt, cuda::std::make_format_args<context_t<CharT>>(args...));
                   return false;
                 }
