...
__host__ __device__ constexpr void test_to_chars(const TestItem& item)
{
  ...
  static_assert(
    cuda::std::is_same_v<cuda::std::to_chars_result, decltype(cuda::std::to_chars((char*) {}, (char*) {}, U{}, Base))>);

  static_assert(noexcept(cuda::std::to_chars((char*) {}, (char*) {}, T{}, Base)));
  static_assert(noexcept(cuda::std::to_chars((char*) {}, (char*) {}, U{}, Base)));

  // Test signed
  if (cuda::std::in_range<T>(item.val))
  {
    ...
    {
      const auto result = cuda::std::to_chars(buff, buff + buff_size, static_cast<T>(item.val), Base);
      assert(result.ec == cuda::std::errc{});
      assert(result.ptr == buff + ref_len);
      assert(str_equal(buff, item.str_signed, buff_size));
    }

    // Check too small buffer
    {
      const auto result = cuda::std::to_chars(buff, buff + ref_len - 1, static_cast<T>(item.val), Base);
    ...
    // Check zero buffer
    {
      const auto result = cuda::std::to_chars(buff, buff, static_cast<T>(item.val), Base);
      assert(result.ec == cuda::std::errc::value_too_large);
  ...
  // Test unsigned
  if (cuda::std::in_range<U>(item.val))
  {
    const auto ref_len = str_len(item.str_unsigned);
    char buff[buff_size]{};

    const auto uval = static_cast<U>(uint64_t(item.val));

    // Check valid buffer size
    {
      char buff[buff_size]{};
      const auto result = cuda::std::to_chars(buff, buff + buff_size, uval, Base);
      assert(result.ec == cuda::std::errc{});
      assert(result.ptr == buff + str_len(item.str_unsigned));
      assert(str_equal(buff, item.str_unsigned, buff_size));
    }

    // Check too small buffer
    {
      const auto result = cuda::std::to_chars(buff, buff + ref_len - 1, uval, Base);
      assert(result.ec == cuda::std::errc::value_too_large);
      assert(result.ptr == buff + ref_len - 1);
    }
    ...
    // Check zero buffer
    {
      const auto result = cuda::std::to_chars(buff, buff, uval, Base);
      assert(result.ec == cuda::std::errc::value_too_large);
      assert(result.ptr == buff);
    }
  }
...
__host__ __device__ constexpr void test_base()
{
  ...
  {
    test_to_chars<int8_t, Base>(item);
    test_to_chars<int16_t, Base>(item);
    test_to_chars<int32_t, Base>(item);
    test_to_chars<int64_t, Base>(item);
  }
}
