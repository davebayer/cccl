...
[[nodiscard]] _CCCL_HOST_DEVICE_API _OutIt __fmt_copy(basic_string_view<_CharT> __str, _OutIt __out_it)
{
  ...
  if constexpr (same_as<decltype(__out_it), back_insert_iterator<__fmt_output_buffer<_OutCharT>>>)
  {
...
{
  // todo: handle __fmt_retarget_buffer once implemented
  if constexpr (same_as<decltype(__out_it), back_insert_iterator<__fmt_output_buffer<_CharT>>>)
  {
