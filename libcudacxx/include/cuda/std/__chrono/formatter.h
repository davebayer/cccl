    ...
    if (*__it == _CharT('%'))
    {
      ...
      switch (*__it)
      {
        ...
        case _CharT('Q'):
          ...
          // - Should it honour the locale setting for the separators?
...
_LIBCPP_HIDE_FROM_ABI auto __format_chrono(
  const _Tp& __value,
  ...
  basic_string_view<_CharT> __chrono_specs)
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        // Note this is a pedantic interpretation of the Standard. A hh_mm_ss
