...
private:
  ...
  __parse_conversion_spec(_ConstIterator& __begin, _ConstIterator __end, __flags __flags)
  {
    ...
    switch (*__begin)
    {
      ...
      case _CharT('p'): // TODO FMT does the formatter require an hour or a time?
      case _CharT('H'):
      case _CharT('I'):
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr void __parse_modifier_E(_ConstIterator& __begin, _ConstIterator __end, __flags __flags)
  {
    ++__begin;
    if (__begin == __end)
    {
  ...
  /// \pre *__begin == 'O'
  /// \post __begin is incremented by one.
  _LIBCPP_HIDE_FROM_ABI constexpr void __parse_modifier_O(_ConstIterator& __begin, _ConstIterator __end, __flags __flags)
  {
    ++__begin;
    if (__begin == __end)
    {
