...
__device__ __uint128_t warp_reduce_min(__uint128_t __v)
{
  ...
  const auto __r3 = __reduce_min_sync(full_warp_mask, __w3);
...
__global__ void test_warp_reduce_min_kernel(const __uint128_t* inputs, __uint128_t* outputs)
{
...
bool test_warp_reduce_min(const char* name, const std::array<__uint128_t, 32>& inputs)
{
  ...
  __uint128_t expected = ~__uint128_t{0};
...
int main()
{
  ...
  for (unsigned word = 0; word < 4; ++word)
  {
    ...
    winner          = (winner & ~mask) | (__uint128_t{1} << (word * 32));
    for (unsigned lower = 0; lower < word; ++lower)
    {
      const auto lower_mask = __uint128_t{0xffff'ffffu} << (lower * 32);
      winner |= lower_mask;
    }
    inputs.fill(baseline);
    inputs[17] = winner;
    std::snprintf(name, sizeof(name), "word %u decides minimum", word);
  ...
  inputs[3]  = baseline;
  inputs[23] = baseline;
  success &= test_warp_reduce_min("tied minimum lanes", inputs);
